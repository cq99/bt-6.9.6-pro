...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		wget -O install.sh https://raw.githubusercontent.com/cq99/bt-6.9.6-pro/master/install/install-ubuntu_6.0.sh && bash install.sh
		exit;
	else
		wget -O install.sh https://raw.githubusercontent.com/cq99/bt-6.9.6-pro/master/install/install-ubuntu_6.0.sh && sudo bash install.sh
		exit;
	fi
fi
...
btpanel_Url=https://raw.githubusercontent.com/cq99/bt-6.9.6-pro/master
...
pip install psutil chardet virtualenv Flask Flask-Session Flask-SocketIO flask-sqlalchemy Pillow gunicorn gevent-websocket paramiko requests
Install_Pillow
Install_psutil
...
wget -O panel.zip $btpanel_Url/install/src/panel6.zip -T 10
wget -O /etc/init.d/bt $btpanel_Url/install/src/bt6.init -T 10
