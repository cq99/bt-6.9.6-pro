...
download_Url= https://raw.githubusercontent.com/cq99/bt-6.9.6-pro/master
MAIN_RETURN=${Red_font}[宝塔面板Bt_Panel防火墙修复脚本]${Font_suffix}
MAIN_RETURNC="${Red_font}提示：修复了被拉黑无法安装插件问题！${Font_suffix}"
MAIN_RETURNE="${Red_font}提示：防火墙无需面板安装，请卸载，直接脚本安装防火墙，请勿在面板升级防火墙！ 防篡改插件已经解除验证，直接面板安装使用！${Font_suffix}"
...
if [[ "${faill}" == "1" ]]; then
    wget -O install.sh  https://raw.githubusercontent.com/cq99/bt-6.9.6-pro/master/btwaf_rule/test/btwaf/install.sh && chmod 755 install.sh && bash install.sh install
	rm -rf install.sh
    main
elif [[ "${faill}" == "2" ]]; then
	main   
